...
// the UI and CLI can report what actually moved instead of just "synced N".
//
// The CLI mirror lives in scripts/copy-specs.mjs (plain JS, can't import TS) —
// keep the two algorithms in step, like the copy logic they sit next to.
...
// a detail-free "updated" rather than breaking the sync.
export function diffSpec(
  ...
  const endpointsChanged = [...next.operations.keys()]
    ...
    .sort();
  const scopesAdded = added(prev.scopes, next.scopes).sort();
